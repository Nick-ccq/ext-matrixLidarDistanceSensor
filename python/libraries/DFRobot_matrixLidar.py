...
class DFRobot_matrixLidar:
	...
	def request_obstacle_sensor_data(self):
		...
		if (len(recv_pkt) >= self.INDEX_RES_DATA + 8) and (recv_pkt[self.INDEX_RES_ERR] == self.ERR_CODE_NONE and recv_pkt[self.INDEX_RES_STATUS] == self.STATUS_SUCCESS):
			...
			self.right = recv_pkt[self.INDEX_RES_DATA + 6] | (recv_pkt[self.INDEX_RES_DATA + 7] << 8)
			return 0
		return 1

	def get_dir(self):
		self.request_obstacle_sensor_data()
		return self._dir
...
class DFRobot_matrixLidar_i2c(DFRobot_matrixLidar):
	def __init__(self,addr,bus_num=0):
		'''!
	...
	def _send_packet(self, pkt):
		'''!
	  	...
	  	'''
		try:
			self.i2c.writeto(self._addr, [0x55] + pkt)
		except:
			pass
